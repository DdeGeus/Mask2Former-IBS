...
def load_mapillary_vistas_newsampling(images_per_class_json):
...
def register_mapillary_vistas_panoptic_newsampling(
    name, metadata, image_root, panoptic_root, semantic_root, panoptic_json, images_per_class_json, instances_json=None
):
    """
    Register a "standard" version of ADE20k panoptic segmentation dataset named `name`.
    The dictionaries in this registered dataset follows detectron2's standard format.
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_mapillary_vistas_newsampling(
            images_per_class_json
        ),
    )
    MetadataCatalog.get(panoptic_name).set(
        panoptic_root=panoptic_root,
...
_PREDEFINED_SPLITS_MAPILLARY_PANOPTIC = {
    "mapillary_vistas_panoptic_train_cropsampling": (
        ...
        "mapillary_vistas/training/images_per_class.json"
    ),
    "mapillary_vistas_panoptic_val_cropsampling": (
...
def get_metadata():
    ...
    images_dir = _PREDEFINED_SPLITS_MAPILLARY_PANOPTIC["mapillary_vistas_panoptic_train_cropsampling"][0]

    images_per_class_json = images_dir.replace("images", "images_per_class.json")
    meta["images_per_cat_json"] = os.path.join(root, images_per_class_json)
...
def register_all_mapillary_vistas_panoptic_newsampling(root):
    metadata = get_metadata()
    for prefix in _PREDEFINED_SPLITS_MAPILLARY_PANOPTIC.keys():
        if "panoptic_val" in prefix:
            image_root, panoptic_root, panoptic_json, semantic_root = _PREDEFINED_SPLITS_MAPILLARY_PANOPTIC[prefix]
            ...
            image_root, panoptic_root, panoptic_json, \
                semantic_root, images_per_class_json = _PREDEFINED_SPLITS_MAPILLARY_PANOPTIC[prefix]

            register_mapillary_vistas_panoptic_newsampling(
                prefix,
                metadata,
                os.path.join(root, image_root),
                os.path.join(root, panoptic_root),
                os.path.join(root, semantic_root),
                os.path.join(root, panoptic_json),
                os.path.join(root, images_per_class_json)
            )


_root = os.getenv("DETECTRON2_DATASETS", "datasets")
register_all_mapillary_vistas_panoptic_newsampling(_root)
